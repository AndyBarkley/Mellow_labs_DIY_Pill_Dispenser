...
const int MORNING_BUTTON_PIN = 21;
long timezone = -5;  // (utc+) TZ in hours Eastern
int daysavetime = 1;
...
int motorPwmValues[NUM_MOTORS] = { 80 };  // PWM speed values for motors
//int thresholdValues[NUM_MOTORS] = {400, 400, 400, 400}; // Sensor threshold values
int thresholdValues[NUM_MOTORS] = { 400 };  // Sensor threshold values
  ...
  <style>
    ...
    * {
    ...
    body.dark-mode #scheduleDialog .days-grid button.active {
     ...
     /* Days Grid - Updated Styles for Time of Day Buttons 
    #scheduleDialog .timeofday-grid {
      display: grid;
      grid-template-columns: repeat(4, 1fr);
      gap: 4px;
      margin-bottom: 8px;
    }
    #scheduleDialog .timeofday-grid button {
      padding: 4px 8px;
      font-size: 0.85rem;
      border: 1px solid #ccc;
      background: transparent;
    ...
    body.dark-mode #scheduleDialog .timeofday-gridd button {
      background: transparent;
      color: #e0e0e0;
      border-color: #555;
    }
    #scheduleDialog .timeofday-grid button.active,
    body.dark-mode #scheduleDialog .timeofday-grid button.active {
      background: var(--accent-color, #007bff);
      color: #fff;
      border-color: var(--accent-color, #007bff);
    }*/
...
<body>
  ...
  <div id="scheduleDialog">
    ...
    <div class="content">
      ...
      <div class="days-grid">
        <button data-day="Morning" onclick="toggleDay(this)">Morning</button>
        <button data-day="Noon" onclick="toggleDay(this)">Noon</button>
        <button data-day="Night onclick="toggleDay(this)">Night</button>
      </div>
    </div>
    <div class="footer">
 ...
 */
void handleRoot() {
  const char* index_site;
  Serial.println("Before checking File"); 
  String stFileContent = "";
  if (LittleFS.exists("/site.txt")) {
    Serial.println("File Found: site.txt");
    File file = LittleFS.open("/site.txt", "r");
    if (!file) {
      Serial.println("Failed to read site.txt.");
      server.send(500, "text/plain", "Failed to open site file");
  }
    stFileContent = file.readString();
    //Serial.println(stFileContent);
    index_site = stFileContent.c_str();
    file.close();
  }else{
    Serial.println("File Not Found: site.txt."); 
  }
  server.send_P(200, "text/html", index_site);
}
...
void handleSetRTC() {
  ...
  configTime(3600*timezone, daysavetime*3600, ntpServer1, ntpServer2);
  struct tm timeinfo;
...
void setup() {
  ...
  pinMode(NIGHT_BUTTON_PIN, INPUT);
  pinMode(MORNING_BUTTON_PIN, INPUT);
...
void loop() {
  ...
  if( digitalRead(NIGHT_BUTTON_PIN) > 0){
    Serial.println("Night Button PRessed");
    currentMotor = 0;
    motorRunning = true;
    stopControl = false;
    previousMillis = millis();
  ...
  if( digitalRead(MORNING_BUTTON_PIN) > 0){
